...
while true; do
  ...
  app_id=$(zenity --list --title="Desinstalar Apps Flatpak" --cancel-label="Sair" --ok-label="Desinstalar" --column="Aplicativos" --height=400 --width=600 "${menu_lines[@]}")

  if [ -n "$app_id" ]; then
   
    app_id=$(echo "$app_id" | cut -d' ' -f1)

    app_info=$(flatpak info "$app_id")

    zenity --text-info --title="Detalhes do Aplicativo" --filename=/dev/stdin <<< "$app_info" --ok-label="Desinstalar"
